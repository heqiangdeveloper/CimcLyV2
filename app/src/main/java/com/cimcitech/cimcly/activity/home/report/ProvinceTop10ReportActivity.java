...
public class ProvinceTop10ReportActivity extends AppCompatActivity{
    ...
    private void showTables(){
        ...
        barChart.setDescription("   省份");// 数据描述
        ...
        if(list != null && list.size() != 0){
            for(int i = 0; i < list.size(); i++){
                xValues.add(list.get(i).getProvinceDesc());
            }
        }
        ...
        barChart.setOnChartValueSelectedListener(new OnChartValueSelectedListener() {
            ...
            public void onValueSelected(Entry e, int dataSetIndex, Highlight h) {
                if(list != null && list.size() != 0){
                    Intent i = new Intent(ProvinceTop10ReportActivity.this,
                            ProvinceTop10ReportDetailActivity.class);
                    i.putExtra("province",list.get(e.getXIndex()).getProvince());
    ...
    private void showGrid(){
        ...
        if(list != null && list.size() != 0){
            for(int i = 0; i < list.size(); i++){
                ...
                ((TextView)linearLayout[i].getChildAt(0)).setText(list.get(i).getProvinceDesc());
